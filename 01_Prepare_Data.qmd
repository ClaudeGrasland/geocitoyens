...
On a désormais toute les informations dans un fichier unique ...

```{r}
saveRDS(mapdoncirc,"don/mapdoncirc.RDS")
```
...
```{r}
table(nchar(mapcom$com_code))
mapdep<-mapcom %>% mutate(dep)
                   group_by(dep) %>%
                       summarise()
